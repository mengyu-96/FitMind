...
版本：v1.4｜日期：2026-09-25｜状态：柔性事实与主动信息获取设计；尚未生成数据库迁移

> v1.3实施范围：本文为跨阶段逻辑模型，不是M0必须一次建完的表清单；首版只建实际调用链需要的源事实、执行索引和治理结构。 具体分期与自主授权以[29](29-微信小程序首版收束与智能体授权.md)为准。
...
M0首先建立用户/用途、消息与任务、fitness_objects及修订/证据/关系、必要投影绑定、操作回执与outbox、简洁计划索引、知识有效版本、导出/删除及最小审计。授权使用、任务预算、租约和检查点保持可靠，物理表可按访问模式合并，不削弱并发约束。

inquiry、interaction_preferences、mandate可保存为源对象kind，服务端固定索引归属、授权效果、状态、版本和期限；首版无需对应独立管理平台。未知属性直接入payload，personal_schema为可选元数据，不建全套Schema管理表和页面。可选标准组投影按需建立；plan_occurrences复杂履约、training_pauses、muscle_states、提醒候选/inbox及报告任务表随对应后续功能迁移，不提前假装可用。

## 10. 关注议程与消息分离（v1.4）

inquiry仍为源对象及必要索引，next_review_at仅表示下次交互可重新评估的最早时间。旧reminder_candidates/inbox/reminder_preferences为未实现的历史候选模型，当前不迁移、不由定时任务生成记录，未来若有明确事项的通知实验再单独设计。

增加最小engagement呈现元数据：user_id、visit_id、engagement_id、inquiry_ref及版本、answer_revision、preference_version、interaction_epoch、呈现占位期限、selected/shown/dismissed/expired等状态与时间。可复用交互状态存储，不建消息中心；关键唯一键保证同visit开场及同主题版本去重。dismissed是呈现关闭，不自动等于议题declined；没有ACK标未知，禁止推断用户已读。

原始呈现元数据默认30天清理，随用户删除级联；必要免问/延期偏好按用户指定范围保留至解除或到期，不靠永久保存曝光日志维持。元数据不复制敏感正文，候选正文短期保留且过期清理，用户展开形成的正常对话遵循对话保留期限。未来再启用外部通知不得把旧站内同意自动迁移为外发授权。

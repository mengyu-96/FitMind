...
版本：v1.3｜日期：2026-09-25｜状态：开放规划器与主动认知循环同步；尚未实现

> v1.3实施范围：M0先实现一个主规划循环和共享能力网关；图中的专项子图是可选拆分模板，后台提醒后置，不以所有子图建齐作为开发前置。 具体分期与自主授权以[29](29-微信小程序首版收束与智能体授权.md)为准。
...
首版向规划器开放29定义的能力族及当前真实可用权限。任务内自主决定读什么、问什么、何时检索/重查、怎样组合写入与视图；不要求固定节点序列或逐工具审批。未知业务表达进入同一个规划循环，而不是只能给模板回复。

apply_changes在服务端统一完成效果验证、意图/委托匹配和提交；已授权动作直接给回执，需要新授权才创建确认中断。持续委托可以跨会话，前台触发时可主动应用，但首版没有离线无人值守自治。检查点、租约、稳定operation_id与task累计预算继续有效。

实现顺序为可用文字任务→可靠源记录→简洁计划与自主调整→议程反馈→数据治理及微信真机闭环；专项子图、复杂任务DAG编辑器及后台回访不提前建设。

## 14. 返回时的主动决策（v1.4）

用户返回事件只让主规划器有机会检查相关性，不强制生成问候、消息或执行新目标。先做便宜的页面/任务/偏好/visit过滤，再按信息价值选择提问、反馈、安静或已有委托内行动。新用户消息优先，旧开场候选按交互代次取消；打开/返回不扩大授权。

不定时扫描离线用户调用模型，next_review_at只是下次交互的评估门槛。已受理业务的恢复、回执与数据治理继续可靠运行。一次自动开场的体验预算不限制用户主动交流中的工具次数或合理深度；能力与主动思考保持开放。
